...
public class PostAPI extends RestApplication{
    ...
    @Path("UpdatePost")
    @Produces(MediaType.APPLICATION_JSON)
    public Response updatePost(@FormParam("postid") String postid,@FormParam("data") String data,@FormParam("type") String type,@FormParam("postDate") String postDate,@FormParam("user") String userId){
        Connection conn=GetConnection.getInstance().getConnection();
        Post p=new Post();
