...
public class User {
  ...
  public void getUser() {
    ...
    for (Friend f : friends) {
      if ((f.getAsker().getId() == this.id || f.getReceiver().getId() == this.id)
          && f.getAccepted()) {
  ...
  public List<User> searchAUser(String keyword) {
    ...
    for (int i = 0; i < users.size(); i++) {
      if (users.get(i).lastname.contains(keyword)) result.add(users.get(i));
      else if (users.get(i).firstname.contains(keyword)) result.add(users.get(i));
    }
    return result;
  }
  ...
  public boolean addASchool(
      String name, String type, String address, String beginDate, String endDate, String graduate) {
  ...
  public List<Post> getPostOfYourFriends() {
    ...
    for (int i = 0; i < friends.size(); i++) {
      if ((friends.get(i).getReceiver().getId() == this.id
              || friends.get(i).getAsker().getId() == this.id)
          && friends.get(i).getAccepted()) {
        friendofyours.add(
            friends.get(i).getReceiver().getId() == this.id
                ? friends.get(i).getAsker()
                : friends.get(i).getReceiver());
      }
    }
  ...
  public boolean updateUser() {
    return new DaoUser().update(this);
  }
}
